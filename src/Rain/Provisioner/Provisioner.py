...
class Provisioner(ProvisionerAmbassador):
    def __init__(self, config):
        ...
        self.logger.log('info', "Creating coordinator")
    ...
    def start_coordinator(self):
        try:
            self.logger.log('info', "Starting coordinator")
    ...
    def create_worker(self, worker_id)  -> None:
        try:
            ...
            # To get the [old workers + new worker] IP, port and status
    ...
    def create_workers(self):
        try:
            ...
            self.ports = self.provisioner.get_workers_ports()
            self.statuses = self.provisioner.get_workers_statuses()
            self.logger.log('info', f"[Created workers]: IPs : {self.ips}, ports: {self.ports}, statuses: {self.statuses}, IDs : {self.ids}")
        except Exception as e:
            self.logger.log('error', f"Error creating workers: {e}")
            return

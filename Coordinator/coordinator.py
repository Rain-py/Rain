...
sys.path.append('../')
from protos import coord_pb2, coord_pb2_grpc, worker_pb2, worker_pb2_grpc, provisioner_pb2, provisioner_pb2_grpc, divider_pb2, divider_pb2_grpc
sys.path.pop()
...
class coordinator(coord_pb2_grpc.coordinatorServicer):
    ...
    def upload(self, request, context):
        ...
        if os.path.exists('coord/'+filepath):
            with open('coord/'+filepath, mode="rb") as f:
                while True:
...
def serve(provisioner_IP, divider_IP, coordinator_stub):
    ...
    server.add_insecure_port('[::]:50052') # open port for communictaion with the coordinator

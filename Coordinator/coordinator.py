...
class coordinator(coord_pb2_grpc.coordinatorServicer):
    ...
    def get_IPs_from_provisioner(self, provisioner_IP):
        ...
        function :
            ...
            and gets the IPs of the workers and thier status (Up or Down).
    ...
    def upload(self, request, context):
        ...
        if os.path.exists("coord/data/" + filepath):
            with open("coord/data/" + filepath, mode="rb") as f:
                while True:
                    chunk = f.read(chunk_size)
                    if chunk:
                        entry_response = coord_pb2.UploadFileResponse(chunk_data=chunk)
        ...
        if target == "worker":
            ...
            with grpc.insecure_channel(ip + ":50051") as channel:
                ...
                response = worker_stub.download(
                    read_file(f"coord/data/X_train_{worker_id}.npy")
                )
                print(" coordinator received: " + response.message)
                response = worker_stub.download(
                    read_file(f"coord/data/y_train_{worker_id}.npy")
                )
                print(" coordinator received: " + response.message)
                response = worker_stub.download(
                    read_file(f"coord/data/{worker_id}.pkl")
                )
                print(" coordinator received: " + response.message)
        elif target == "divider":
            ...
            with grpc.insecure_channel(ip + ":50053") as channel:
                ...
                response = divider_stub.download(
                    read_file(f"coord/data/{worker_id}_trained.pkl")
                )
                print(" coordinator received: " + response.message)

    def execute(self, worker_id, ip):
        with grpc.insecure_channel(ip + ":50051") as channel:
            worker_stub = worker_pb2_grpc.workerStub(
                channel
            )  # interface for the grpc client(worker)

            filename = "Algo"
            extension = ".py"

            print(str(worker_id))
            response = worker_stub.Execute(
                worker_pb2.executeData(
                    filename=filename, extension=extension, worker_id=str(worker_id)
                )
            )
            print(" coordinator received: " + response.message)
...
def read_file(filepath, chunk_size=1024):
    """
    function :
        divide the file into chuncks and send it as stream of small chunks using yield.
    input : filepath
...
def serve(provisioner_IP, divider_IP, coordinator_stub):
    ...
    server.add_insecure_port(
        "[::]:50052"
    )  # open port for communictaion with the coordinator
    ...
    # get the data files from the divider
    # wait for the divider to send the data files
...
if __name__ == "__main__":
    ...
    coordinator_ = coordinator(divider_IP)
    serve(provisioner_IP, divider_IP, coordinator_)
